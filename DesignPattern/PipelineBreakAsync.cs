...
namespace DesignPattern.PipelineBreakAsync
{
    ...
    public class Operation<T> : IOperation<T>
    {
        ...
        private IOperation<T> _next = new Operation<T>(data => false);
        ...
        bool IOperation<T>.Invoke(T data)
        {
            if (_action(data))
                return _next.Invoke(data);
    ...
    {
        private IOperation<string> _next = new Operation<string>(x => false);
        ...
        bool IOperation<string>.Invoke(string data)
        {
            Task.Run(() =>
            {
                Console.WriteLine("Writing data to the disk...");
    ...
    public class ProductOperation : IOperation<Client>
    {
        ...
        bool IOperation<Client>.Invoke(Client data)
        {
            Console.WriteLine("ProductOperation => {0}", data.Name);
            return _next.Invoke(data);
        }
    ...
    {
        private IOperation<Client> _next = new Operation<Client>(x => false);
